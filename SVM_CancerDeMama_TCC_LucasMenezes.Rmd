...
# Treinar usando caret - SVM Polynomial
svm_caret_fit_poly <- train(Class ~ ., data = trainData[,-11],
                            method = "svmPoly",
                            trControl = trainControl(method = "cv", number = 10),
                            tuneGrid = expand.grid(
                              degree = 2,    # Grau do polinômio (ajuste conforme quiser)
                              scale = 0.1,   # Fator de ajuste (análogo ao gamma no radial)
                              C = 2          # Parâmetro de custo
                            ))

# Importância das variáveis
varImpPlot <- varImp(svm_caret_fit_poly)
print(varImpPlot)
plot(varImpPlot)
